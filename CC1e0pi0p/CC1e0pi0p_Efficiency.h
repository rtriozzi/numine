...
namespace ana {
    ...
    const SpillMultiVar kCC1e0pi0pSignal_TrueNeutrinoEnergy([](const caf::SRSpillProxy* sr)-> std::vector<double> {

        std::vector<double> trueEnergies;

        for (auto const& nu : sr->mc.nu) { 
            if (kIsTrueCC1e0pi0p(nu)) {
    ...
    const SpillMultiVar kCC1e0pi0pSignal_TrueElectronEnergy([](const caf::SRSpillProxy* sr)-> std::vector<double> {
    ...
    SpillMultiVar kCC1e0pi0pSignal_TrueNeutrinoEnergy_MakeSelectionStep(const Cut& cut)
    {
                ...
                if (kIsTrueCC1e0pi0p(nu)) {
                    ...
                    for (auto const& islc : sr->slc) {
                        if ((islc.truth.index == nu.index) &&  ///< same index, to account for pile-up
                            (kTrueCC1e0pi0p(&islc)) &&           ///< signal slice
    ...
    SpillMultiVar kCC1e0pi0pSignal_TrueElectronEnergy_MakeSelectionStep(const Cut& cut)
    {
        return SpillMultiVar([cut](const caf::SRSpillProxy* sr) -> std::vector<double> {
            bool trueNeutrinoWasCounted;
            std::vector<double> selectedEnergies;
