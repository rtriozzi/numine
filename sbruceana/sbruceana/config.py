...
CC1E0PI_CATEGORIES_REDUX = [
  ...
  Category(
    label = '$\\nu_{\\mu}$CC$\\pi^0$',
    color = 'violet',
    mask  = lambda df: (df.numu == 1) & (df.CC == 1) & (df.trueFV == 1) & (df.ispi0 == 1),
  ),
  Category(
    label = '$\\nu_{\\mu}$CC',
    color = 'darkorchid',
    mask  = lambda df: (df.numu == 1) & (df.CC == 1) & (df.trueFV == 1) & (df.ispi0 == 0),
  ),
  Category(
    label = '$\\nu$NC',
    color = 'pink',
    mask  = lambda df: (df.CC == 0) & (df.trueFV == 1),
  ),
  Category(
    label = 'OoFV',
    color = 'cyan',
    mask  = lambda df: (df.trueOOFV == 1),
  ),
  Category(
    label = 'cosmics',
    color = 'dodgerblue',
...
CC1MU0PI_CATEGORIES = [
  ...
  Category(
    label = '$\\nu_{\\mu}$CC$\\pi^{\\pm}$',
    color = 'violet',
    mask  = lambda df: (df.signal != 1) & (df.numu == 1) & (df.CC == 1) & (df.trueFV == 1) & (df.ischpi == 1),
  ),
  Category(
    label = '$\\nu_{\\mu}$CC',
    color = 'palevioletred',
    mask  = lambda df: (df.signal != 1) & (df.numu == 1) & (df.CC == 1) & (df.trueFV == 1) & (df.ischpi == 0),
  ),
  Category(
    label = '$\\nu$NC',
    color = 'pink',
...
TRUTH_CATEGORIES_CCNC = [
    Category(
        label = '$\\overline{\\nu}$CC',
        color = 'teal',
        mask  = lambda df: (df.pdg < 0) & (df.iscc == 1),
    ),
    Category(
        label = '$\\overline{\\nu}$NC',
        color = 'darkgoldenrod',
        mask  = lambda df: (df.pdg < 0) & (df.iscc == 0),
    ),
    Category(
        label = '$\\nu$CC',
        color = 'steelblue',
...
TRUTH_CATEGORIES_ANTINU = [
    Category(
        label = 'QE',
        color = 'steelblue',
        mask  = lambda df: (df.pdg > 0) & (df.iscc == 1) & (df.genie_mode == kQE),
    ),
    Category(
        label = 'MEC',
        color = 'goldenrod',
        mask  = lambda df: (df.pdg > 0) & (df.iscc == 1) & (df.genie_mode == kMEC),
    ),
    Category(
        label = 'RES',
        color = 'orangered',
        mask  = lambda df: (df.pdg > 0) & (df.iscc == 1) & (df.genie_mode == kRes),
    ),
    Category(
        label = 'DIS',
        color = 'orchid',
        mask  = lambda df: (df.pdg > 0) & (df.iscc == 1) & (df.genie_mode == kDIS),
    ),
    Category(
        label = 'Other',
        color = 'gray',
        mask  = lambda df: (df.pdg > 0) & (df.iscc == 1) & (~df.genie_mode.isin([kQE, kMEC, kRes, kDIS])),
    ),
    Category(
        label = '$\\overline{\\nu}',
        color = 'black',
