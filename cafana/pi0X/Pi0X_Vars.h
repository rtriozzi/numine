...
namespace ana {
    ...
    const Var kNuGraph_NShowerPFPs_AboveThreshold([](const caf::SRSliceProxy *slc) -> int {
        int kNPFPs(0);
    ...
    double kCosVertexAngle(const caf::SRSliceProxy* slc, unsigned int iPFP) {
        ...
        if (std::isnan(slc->reco.pfp[iPFP].shw.start.x) || std::isnan(slc->reco.pfp[iPFP].shw.start.y) || std::isnan(slc->reco.pfp[iPFP].shw.start.z)) return -5.;
        if (std::isnan(slc->reco.pfp[iPFP].shw.dir.x) || std::isnan(slc->reco.pfp[iPFP].shw.dir.y) || std::isnan(slc->reco.pfp[iPFP].shw.dir.z)) return -5.;

        TVector3 shwDir(slc->reco.pfp[iPFP].shw.dir.x,
                        slc->reco.pfp[iPFP].shw.dir.y,
                        slc->reco.pfp[iPFP].shw.dir.z);

        TVector3 displacement(slc->reco.pfp[iPFP].shw.start.x - slc->vertex.x,
                            slc->reco.pfp[iPFP].shw.start.y - slc->vertex.y,
                            slc->reco.pfp[iPFP].shw.start.z - slc->vertex.z);

        if (shwDir.Mag() == 0. || displacement.Mag() == 0.) return -5.;
    ...
    const Var kLargestRecoShower_TrueEnergy([](const caf::SRSliceProxy* slc) -> double {
        const int largestShwIdx = kLargestRecoShowerIdx(slc);
        if(largestShwIdx == -1) return -5;
        if(std::isnan(slc->reco.pfp[largestShwIdx].shw.truth.p.startE)) return -5;

        return slc->reco.pfp[largestShwIdx].shw.truth.p.startE;
    });

    const Var kLargestRecoShower_CosVertexAngle([](const caf::SRSliceProxy* slc) -> double {
        const int largestShwIdx = kLargestRecoShowerIdx(slc);
        if (largestShwIdx == -1) return -5.;
    ...
    const Var kSubleadRecoShower_TrueEnergy([](const caf::SRSliceProxy* slc) -> double {
        const int subleadShwIdx = kSubleadingRecoShowerIdx(slc);
        if (subleadShwIdx == -1) return -5;
        if (std::isnan(slc->reco.pfp[subleadShwIdx].shw.truth.p.startE)) return -5;

        return slc->reco.pfp[subleadShwIdx].shw.truth.p.startE;
    });

    const Var kSubleadRecoShower_CosVertexAngle([](const caf::SRSliceProxy* slc) -> double {
        const int subleadShwIdx = kSubleadingRecoShowerIdx(slc);
        if (subleadShwIdx == -1) return -5.;
